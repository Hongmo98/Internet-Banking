...
module.exports = {
  queryAccountInformation: async (req, res, next) => {
    ...
    if (
      req.headers["partnercode"] === infoBank.codePGP ||
      req.headers["partnercode"] === codeRSA
    ) {
      let time = +req.headers["headerts"].toString() + 60000;
      if (+time > +timeStamp) {
        if (bcrypt.compareSync(sig, req.headers["headersig"])) {
          try {
            let accountNumber = await bankAccount.findOne({
  ...
  transactionAccount: async (req, res, next) => {
    ...
    if (
      ...
      typeof accountReceiver === undefined ||
    ...
    if (
      req.headers["partnercode"] === infoBank.codePGP ||
      req.headers["partnercode"] === infoBank.codeRSA
    ) {
      ...
      if (+time > +timeStamp) {
        if (bcrypt.compareSync(sig, req.headers["headersig"])) {
          try {
            let connectBank = await linkedBank.findOne({
              partnerMe: req.headers["partnercode"],
            });

            const {
              keys: [privateKey],
            } = await openpgp.key.readArmored(infoBank.code);
            await privateKey.decrypt(infoBank.password);

            const verified = await openpgp.verify({
              message: openpgp.cleartext.fromText("Nap tien "),
              signature: await openpgp.signature.readArmored(signature),
              publicKeys: (await openpgp.key.readArmored(connectBank.public))
                .keys,
            });
            ...
            if (valid) {
              ...
              userReceiver = await bankAccount.findOne({
              ...
              if (typeSend === true) {
                ...
                userReceiver.currentBalance =
                  +userReceiver.currentBalance + +money - fee;
  ...
  linkBankPartnerRsa: async (req, res, next) => {
    ...
    console.log(
      crypto
        .createHash("sha256")
        .update(timestamp + _data + config.SECRET_KEY)
        .digest("hex")
    );
    axios({
      method: "get",
    ...
    try {
      ...
      client.get(userId, async function (err, value) {
        ...
        if (value === code) {
          if (nameBank === "NKLBank") {
            const partner_code =
              "MtcwLbASeIXVnKurQCHrDCmsTEsBD7rQ44wHsEWjWtl8k";
            let infoBank = await information.findOne({
              secrekey: config.SECRET_KEY,
            });
              ...
              if (
                ...
                (async () => {
                  ...
                  axios
                    ...
                    .then(async function (response) {
                      let getMoney = await getSenderMoney(
                        response.data,
                        amountMoney,
                        typeSend,
                        userSender
                      ...
                      if (getMoney === "error") {
                        next({
                          error: {
                            message: "current balance not enough to transfer",
                            code: 422,
                          },
                        });
                        return;
                      ...
                      await link.save();
                      let re = await receiverInfo.findOne({
                        numberAccount: receiver,
                      });
                      let type = false;
                      if (re === null) {
                        type = true;

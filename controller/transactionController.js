...
const client = redis.createClient(
	"13088",
	"redis-13088.c8.us-east-1-2.ec2.cloud.redislabs.com",
	{ no_ready_check: true }
);
...
module.exports = {
	...
	requestReceiver: async (req, res, next) => {
		...
		if (typeSend === true || typeSend === false) {
			...
			mailer
				.sentMailer("mpbank.dack@gmail.com", { email }, "MPBank Transfer", a)
				.then(async (json) => {
					console.log(token);
					console.log(json);
	...
	transferInternal: async (req, res, next) => {
		...
		if (typeof req.body.idTransaction === "undefined") {
			next({ error: { message: "Invalid data", code: 422 } });
			return;
		}
		...
		mailer
			.sentMailer("mpbank.dack@gmail.com", { email }, "transfer", token)
			.then(async (json) => {
	...
	verifyOTP: async (req, res, next) => {
		...
		try {
			client.get(userId, async (err, value) => {
				...
				{
					if (value === code) {
						let userReceiver = await bankAccount.findOne({
						...
						if (+userSender.currentBalance > service) {
							if (typeSend === false) {
								userReceiver.currentBalance =
									+userReceiver.currentBalance + money - feeTransfer;

								userSender.currentBalance = +userSender.currentBalance - money;
							} else {
								userReceiver.currentBalance =
									+userReceiver.currentBalance + money;

								userSender.currentBalance =
									+userSender.currentBalance - money - feeTransfer;
							}
							...
							if (typeTransaction === "INDEPT") {
								let deptTra = await deptInformation.findById({
									_id: ObjectId(idRemind),
								});
							...
							next({
								error: {
									message: "current balance isn't to transfer money",
									code: 422,
								},
							});
						}
	...
	saveReceive: async (req, res, next) => {
		...
		try {
			...
			let number = await receiverInfo.findOne({
			...
			let object = {
				msg: "save information receiver in success",
				saveInfo,
				link,
			};
	...
	updateReceiver: async (req, res, next) => {
		...
		try {
			let receiver = await receiverInfo.findOneAndUpdate(
	...
	showDeptRemindUnPay: async (req, res, next) => {
		...
		try {
			...
			let dept = await deptInformation.aggregate([{ $match: conditionQuery }]);
			console.log(dept);
			if (dept.length > 0) {
				res.status(200).json({
					result: dept,
	...
	showDeptRemind: async (req, res, next) => {
		...
		try {
			...
			let dept = await deptInformation.find({
				bankAccountSender: sender.accountNumber,
				isDelete: false,
			});
	...
	deleteReminder: async (req, res, next) => {
		...
		try {
			...
			return res
				.status(200)
				.json({ msg: "delete success", reminder, deptUser });
		} catch (err) {
			next(err);
		}
	},
	...
	updateReminder: async (req, res, next) => {
		...
		delete objectUpdate["id"];
		try {
			let e = await deptInformation.findOneAndUpdate(
				{ _id: ObjectId(id), userId: ObjectId(userId) },
				objectUpdate
			);
...
formatEmail = (token) => {
	...
	let a =
		"<p>You are making a bank transfer in MPBank Internet Banking. </b> <ul><li> Your transaction code is <h1>" +
		token +
		"</h1></li> <li>" +
		object +
		"</li> <li>" +
